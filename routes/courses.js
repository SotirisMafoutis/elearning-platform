...
router.get('/courses/:id', authenticate, (req, res) => {
  const course = db.prepare(`SELECT c.*, cat.name AS category_name, u.full_name AS instructor_name
                              FROM courses c LEFT JOIN categories cat ON cat.id = c.category_id
                              JOIN users u ON u.id = c.instructor_id WHERE c.id = ?`).get(req.params.id);
  ...
  if (req.user.role === 'learner') {
    ...
    if (enrollment) {
      progress = db.prepare('SELECT module_id, completed, completed_at FROM module_progress WHERE enrollment_id=?').all(enrollment.id);
    }
  }
...
router.post('/courses', authenticate, requireRole('instructor'), (req, res) => {
  ...
  const info = db.prepare(`INSERT INTO courses (title, description, category_id, instructor_id, prerequisites, status)
                            VALUES (?,?,?,?,?,'draft')`)
...
router.put('/courses/:id', authenticate, requireRole('instructor'), (req, res) => {
  ...
  if (!course || course.instructor_id !== req.user.id) return res.status(404).json({ error: 'Το μάθημα δεν βρέθηκε.' });
...
// Το κείμενο θεωρίας είναι συμπληρωματικό υλικό, όχι δομικό στοιχείο του μαθήματος —
// γι' αυτό επιτρέπεται η επεξεργασία του ανεξαρτήτως κατάστασης (ακόμη και σε δημοσιευμένο μάθημα).
router.put('/courses/:id/content', authenticate, requireRole('instructor'), (req, res) => {
  const course = db.prepare('SELECT * FROM courses WHERE id=?').get(req.params.id);
  if (!course || course.instructor_id !== req.user.id) return res.status(404).json({ error: 'Το μάθημα δεν βρέθηκε.' });
  const { content_text } = req.body || {};
  db.prepare('UPDATE courses SET content_text=? WHERE id=?').run(content_text ?? '', course.id);
  res.json({ message: 'Η θεωρία του μαθήματος ενημερώθηκε.' });
});
...
router.post('/courses/:id/submit', authenticate, requireRole('instructor'), (req, res) => {
  ...
  if (moduleCount === 0) return res.status(400).json({ error: 'Προσθέστε τουλάχιστον μία ενότητα πριν την υποβολή για έγκριση.' });
  db.prepare(`UPDATE courses SET status='pending' WHERE id=?`).run(course.id);
...
router.post('/courses/:id/modules', authenticate, requireRole('instructor'), (req, res) => {
  ...
  if (!title) return res.status(400).json({ error: 'Ο τίτλος ενότητας είναι υποχρεωτικός.' });
  const nextOrder = db.prepare('SELECT COALESCE(MAX(order_index),0)+1 n FROM modules WHERE course_id=?').get(course.id).n;
...
router.post('/courses/:id/enroll', authenticate, requireRole('learner'), (req, res) => {
  ...
  if (!course) return res.status(404).json({ error: 'Το μάθημα δεν βρέθηκε ή δεν είναι διαθέσιμο.' });
  const existing = db.prepare('SELECT id FROM enrollments WHERE user_id=? AND course_id=?').get(req.user.id, course.id);
  if (existing) return res.status(409).json({ error: 'Είστε ήδη εγγεγραμμένος/η σε αυτό το μάθημα.' });
  const info = db.prepare('INSERT INTO enrollments (user_id, course_id) VALUES (?,?)').run(req.user.id, course.id);
  res.status(201).json({ id: info.lastInsertRowid });
});
...
router.post('/modules/:id/complete', authenticate, requireRole('learner'), (req, res) => {
  ...
  if (!mod) return res.status(404).json({ error: 'Η ενότητα δεν βρέθηκε.' });
  const enrollment = db.prepare('SELECT * FROM enrollments WHERE user_id=? AND course_id=?').get(req.user.id, mod.course_id);
  if (!enrollment) return res.status(403).json({ error: 'Δεν είστε εγγεγραμμένος/η σε αυτό το μάθημα.' });

  const modules = db.prepare('SELECT * FROM modules WHERE course_id=? ORDER BY order_index').all(mod.course_id);
  const idx = modules.findIndex(m => m.id === mod.id);
  if (idx > 0) {
    const prev = db.prepare('SELECT completed FROM module_progress WHERE enrollment_id=? AND module_id=?')
  ...
  db.prepare(`INSERT INTO module_progress (enrollment_id, module_id, completed, completed_at) VALUES (?,?,1, datetime('now'))
              ON CONFLICT(enrollment_id, module_id) DO UPDATE SET completed=1, completed_at=datetime('now')`)
...
router.get('/me/dashboard', authenticate, requireRole('learner'), (req, res) => {
  const enrollments = db.prepare(`
    SELECT e.*, c.title,
      (SELECT COUNT(*) FROM modules WHERE course_id=c.id) AS total_modules,
      (SELECT COUNT(*) FROM module_progress mp WHERE mp.enrollment_id=e.id AND mp.completed=1) AS done_modules
    FROM enrollments e JOIN courses c ON c.id=e.course_id WHERE e.user_id=?`).all(req.user.id);

  const avgScoreRow = db.prepare('SELECT AVG(score) avg FROM quiz_attempts WHERE user_id=? AND submitted_at IS NOT NULL').get(req.user.id);
  const certificates = db.prepare(`SELECT cert.*, c.title AS course_title FROM certificates cert
                                    JOIN courses c ON c.id=cert.course_id WHERE cert.user_id=?`).all(req.user.id);
